...
UDPHelper::UDPHelper(int role, QObject *parent) : QObject(parent) {
  ...
  QObject::connect(udp_socket, &QUdpSocket::readyRead, this,
                   &UDPHelper::handle_udp_socket_ready_read);
  QObject::connect(udp_socket, &QAbstractSocket::errorOccurred, this,
                   &UDPHelper::handle_udp_socket_error);
}

UDPHelper::UDPHelper(int role, const QHostAddress &address, int port,
                     QObject *parent)
    : QObject(parent) {
  udp_socket = new QUdpSocket(this);
...
qint64 UDPHelper::write(const char *data, qint64 len,
                        const QHostAddress &address, int port) {
  return udp_socket->writeDatagram(data, len, address, port);
}

qint64 UDPHelper::write(const char *data, const QHostAddress &address,
                        int port) {
  int len = strlen(data);
  return udp_socket->writeDatagram(data, len, address, port);
}
...
void UDPHelper::setAutoWrite(const QByteArray &data) { m_auto_data = data; }
...
void UDPHelper::handle_udp_socket_error(
