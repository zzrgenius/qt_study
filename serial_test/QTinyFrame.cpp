...
#define CKSUM_RESET(cksum)                                                     \
  do {                                                                         \
    (cksum) = QTinyFrame::TF_CksumStart();                                     \
  } while (0)
#define CKSUM_ADD(cksum, byte)                                                 \
  do {                                                                         \
    (cksum) = QTinyFrame::TF_CksumAdd((cksum), (byte));                        \
  } while (0)
#define CKSUM_FINALIZE(cksum)                                                  \
  do {                                                                         \
    (cksum) = QTinyFrame::TF_CksumEnd((cksum));                                \
  } while (0)
...
void QTinyFrame::TF_WriteImpl(TinyFrame *tf, const uint8_t *buff,
  ...
  //  }
  //  if (tem_serial->isOpen())
  //    tem_serial->write((const char *)buff, len);
}
...
inline uint8_t crc8_bits(uint8_t data) {
  uint8_t crc = 0;
  if (data & 1)
    crc ^= 0x5e;
  if (data & 2)
...
TF_CKSUM QTinyFrame::TF_CksumStart(void) { return 0; }

TF_CKSUM QTinyFrame::TF_CksumAdd(TF_CKSUM cksum, uint8_t byte) {
  return (cksum >> 8) ^ crc16_table[(cksum ^ byte) & 0xff];
}

TF_CKSUM QTinyFrame::TF_CksumEnd(TF_CKSUM cksum) { return cksum; }
...
TF_CKSUM QTinyFrame::TF_CksumStart(void) { return (TF_CKSUM)0xFFFFFFFF; }

TF_CKSUM QTinyFrame::TF_CksumAdd(TF_CKSUM cksum, uint8_t byte) {
  return crc32_table[((cksum) ^ ((uint8_t)byte)) & 0xff] ^ ((cksum) >> 8);
}

TF_CKSUM QTinyFrame::TF_CksumEnd(TF_CKSUM cksum) { return (TF_CKSUM)~cksum; }
...
bool QTinyFrame::TF_ClaimTx(TinyFrame *tf) {
  this->frame_mutex.lock();
  return 1;
}
void QTinyFrame::TF_ReleaseTx(TinyFrame *tf) { this->frame_mutex.unlock(); }
 ...
 * @param tf - instance
 */
void QTinyFrame::TF_SendFrame_End(TinyFrame *tf) {
 ...
 * @return true if sent
 */
bool QTinyFrame::TF_SendFrame(TinyFrame *tf, TF_Msg *msg, TF_Listener listener,
                              TF_Listener_Timeout ftimeout, TF_TICKS timeout) {
  TF_TRY(QTinyFrame::TF_SendFrame_Begin(tf, msg, listener, ftimeout, timeout));
  if (msg->len == 0 || msg->data != NULL) {
    ...
    QTinyFrame::TF_SendFrame_Chunk(tf, msg->data, msg->len);
    QTinyFrame::TF_SendFrame_End(tf);
  }
  return true;
}

/** Reset ID listener's timeout to the original value */
inline void QTinyFrame::renew_id_listener(struct TF_IdListener_ *lst) {
  lst->timeout = lst->timeout_max;
}

/** Notify callback about ID listener's demise & let it free any resources in
 * userdata */
void QTinyFrame::cleanup_id_listener(TinyFrame *tf, TF_COUNT i,
...
inline void QTinyFrame::cleanup_type_listener(TinyFrame *tf, TF_COUNT i,
...
inline void
QTinyFrame::cleanup_generic_listener(TinyFrame *tf, TF_COUNT i,
...
bool QTinyFrame::TF_AddIdListener(TinyFrame *tf, TF_Msg *msg, TF_Listener cb,
...
bool QTinyFrame::TF_AddTypeListener(TinyFrame *tf, TF_TYPE frame_type,
                                    TF_Listener cb) {
  TF_COUNT i;
  struct TF_TypeListener_ *lst;
...
bool QTinyFrame::TF_AddGenericListener(TinyFrame *tf, TF_Listener cb) {
  TF_COUNT i;
  struct TF_GenericListener_ *lst;
...
bool QTinyFrame::TF_RemoveIdListener(TinyFrame *tf, TF_ID frame_id) {
  TF_COUNT i;
  struct TF_IdListener_ *lst;
  for (i = 0; i < tf->count_id_lst; i++) {
    ...
    if (lst->fn != NULL && lst->id == frame_id) {
      QTinyFrame::cleanup_id_listener(tf, i, lst);
      return true;
    }
  }
...
bool QTinyFrame::TF_RemoveTypeListener(TinyFrame *tf, TF_TYPE type) {
  TF_COUNT i;
  struct TF_TypeListener_ *lst;
...
bool QTinyFrame::TF_RemoveGenericListener(TinyFrame *tf, TF_Listener cb) {
  TF_COUNT i;
  struct TF_GenericListener_ *lst;
...
void QTinyFrame::TF_HandleReceivedMessage(TinyFrame *tf) {
  TF_COUNT i;
...
bool QTinyFrame::TF_RenewIdListener(TinyFrame *tf, TF_ID id) {
  TF_COUNT i;
  struct TF_IdListener_ *lst;
  for (i = 0; i < tf->count_id_lst; i++) {
    lst = &tf->id_listeners[i];
...
void QTinyFrame::TF_Accept(TinyFrame *tf, const uint8_t *buffer,
                           uint32_t count) {
  uint32_t i;
  for (i = 0; i < count; i++) {
...
void QTinyFrame::TF_ResetParser(TinyFrame *tf) {
...
void QTinyFrame::pars_begin_frame(TinyFrame *tf) {
  // Reset state vars
...
#if !TF_USE_SOF_BYTE
  if (tf->state == TFState_SOF) {
    QTinyFrame::pars_begin_frame(tf);
  }
#endif
 ...
 */
inline uint32_t QTinyFrame::TF_ComposeHead(TinyFrame *tf, uint8_t *outbuff,
                                           TF_Msg *msg) {
 ...
 */
inline uint32_t QTinyFrame::TF_ComposeBody(uint8_t *outbuff,
 ...
 * @return nr of bytes in outbuff used
 */
inline uint32_t QTinyFrame::TF_ComposeTail(uint8_t *outbuff, TF_CKSUM *cksum) {
  int8_t si = 0; // signed small int
  uint8_t b = 0;
  uint32_t pos = 0;
 ...
 */
bool QTinyFrame::TF_SendFrame_Begin(TinyFrame *tf, TF_Msg *msg,
 ...
 * @param length - count
 */
void QTinyFrame::TF_SendFrame_Chunk(TinyFrame *tf, const uint8_t *buff,
                                    uint32_t length) {
  uint32_t remain;
  uint32_t chunk;
  uint32_t sent = 0;
...
bool QTinyFrame::TF_Send(TinyFrame *tf, TF_Msg *msg) {
  return TF_SendFrame(tf, msg, NULL, NULL, 0);
}

/** send without listener and struct */
bool QTinyFrame::TF_SendSimple(TinyFrame *tf, TF_TYPE type, const uint8_t *data,
                               TF_LEN len) {
  TF_Msg msg;
  TF_ClearMsg(&msg);
  msg.type = type;
...
bool QTinyFrame::TF_QuerySimple(TinyFrame *tf, TF_TYPE type,
...
bool QTinyFrame::TF_Query(TinyFrame *tf, TF_Msg *msg, TF_Listener listener,
                          TF_Listener_Timeout ftimeout, TF_TICKS timeout) {
  return TF_SendFrame(tf, msg, listener, ftimeout, timeout);
}
 ...
 * for responses */
bool QTinyFrame::TF_Respond(TinyFrame *tf, TF_Msg *msg) {
...
bool QTinyFrame::TF_Send_Multipart(TinyFrame *tf, TF_Msg *msg) {
  msg->data = NULL;
  return TF_Send(tf, msg);
}

bool QTinyFrame::TF_SendSimple_Multipart(TinyFrame *tf, TF_TYPE type,
                                         TF_LEN len) {
  return TF_SendSimple(tf, type, NULL, len);
}

bool QTinyFrame::TF_QuerySimple_Multipart(TinyFrame *tf, TF_TYPE type,
                                          TF_LEN len, TF_Listener listener,
                                          TF_Listener_Timeout ftimeout,
                                          TF_TICKS timeout) {
  return QTinyFrame::TF_QuerySimple(tf, type, NULL, len, listener, ftimeout,
                                    timeout);
}

bool QTinyFrame::TF_Query_Multipart(TinyFrame *tf, TF_Msg *msg,
                                    TF_Listener listener,
                                    TF_Listener_Timeout ftimeout,
                                    TF_TICKS timeout) {
  msg->data = NULL;
  return QTinyFrame::TF_Query(tf, msg, listener, ftimeout, timeout);
}

void QTinyFrame::TF_Respond_Multipart(TinyFrame *tf, TF_Msg *msg) {
  msg->data = NULL;
  QTinyFrame::TF_Respond(tf, msg);
}

void QTinyFrame::TF_Multipart_Payload(TinyFrame *tf, const uint8_t *buff,
                                      uint32_t length) {
  QTinyFrame::TF_SendFrame_Chunk(tf, buff, length);
}

void QTinyFrame::TF_Multipart_Close(TinyFrame *tf) { TF_SendFrame_End(tf); }

// endregion Sending API funcs - multipart

/** Timebase hook - for timeouts */
void QTinyFrame::TF_Tick(TinyFrame *tf) {
  TF_COUNT i;
  struct TF_IdListener_ *lst;
