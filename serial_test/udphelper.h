...
class UDPHelper : public QObject {
 ...
 public:
  ...
  UDPHelper(int role, const QHostAddress &address, int port,
  ...
  qint64 write(const char *data, qint64 len, const QHostAddress &address,
               int port);
  qint64 write(const char *data, const QHostAddress &address, int port);
  inline qint64 write(const QByteArray &data, const QHostAddress &address,
                      int port) {
