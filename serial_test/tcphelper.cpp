...
TCPHelper::TCPHelper(int role, QObject *parent) : QObject(parent) {
  ...
  if (role == Client) {
    ...
    QObject::connect(socket, &QTcpSocket::readyRead, this,
                     &TCPHelper::handle_tcp_socket_ready_read);
  }
}
...
void TCPHelper::handle_tcp_server_error(
...
void TCPHelper::handle_tcp_socket_disconnected() {
  QTcpSocket *socket = qobject_cast<QTcpSocket *>(sender());
  emit disconnected(
      this->ip, QHostAddress(socket->peerAddress().toIPv4Address()).toString(),
      this->port, socket->peerPort());
}

void TCPHelper::handle_tcp_socket_state_changed(
    QAbstractSocket::SocketState state) {}

void TCPHelper::handle_tcp_socket_error(
    QAbstractSocket::SocketError socketError) {
  int err = (int)socketError;
  emit errorOccurred(err);
}
